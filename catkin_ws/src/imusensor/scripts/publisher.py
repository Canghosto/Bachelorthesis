...
imuValue_pf = rospy.Publisher('/imu_event_pf', msg.ImuEvent, queue_size=10)
...
while True:
    for data in dataParser.Parser(data_source, udpStatus):
        ...
        elif isinstance(data, dataParser.ImuEvent_PF):
            imuValue_pf.publish(rospy.Time.now(), data.seqNum, data.imuId, msg.AccelGyro(data.accel_value, data.gyro_value))
